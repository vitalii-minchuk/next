  ...
  return (
    <Fragment>
      <nav className="block xs:hidden">
        ...
        w-full divide-y divide-indigo-300 transition-all ease-in duration-400 bg-indigo-200`}
        >
          {navigation.map(link => (
            <li key={link.id}>
              <Link href={link.path}>
                <a onClick={() => setOpen(false)} className="block w-[96%] text-center mx-auto my-1 transition-all duration-300 rounded-md py-2 hover:text-white hover:bg-opacity-30 hover:bg-indigo-500"
                >
                  {link.title}
                </a>
              </Link>
